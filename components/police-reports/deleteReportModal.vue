<template>
  <!-- Delete House Modal -->
  <div>
    <div v-if="getIsDeleteReportPopupOpen" class="modal fade" :class= "{show: getIsDeleteReportPopupOpen}"
        :style="{display: getIsDeleteReportPopupOpen ? 'block' : 'none'}"
        id="deleteReportModal" tabindex="-1"  aria-hidden="true">
      <div class="modal-dialog alert-modal danger modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="alert-footer">
              <button class="btn" data-bs-dismiss="modal" data-bs-target="#deleteHouseAlert" @click="closeDeleteReportModalPopUp()">Close</button>
              <button
              class="btn btn-danger"
              data-bs-dismiss="modal"
              @click="deleteReportId()"
              >
  ...
  <div v-if="getIsDeleteReportPopupOpen" class="modal-backdrop fade show" />
  </div>
...
export default {
  ...
  computed: {
    ...mapGetters({
        getIsDeleteReportPopupOpen: 'report/getIsDeleteReportPopupOpen',
        getDeleteReportId: 'report/getDeleteReportId',
        getDeleteReportType: 'report/getDeleteReportType',

    })
  },
  methods: {
    closeDeleteReportModalPopUp () {
      this.$store.commit('report/setIsDeleteReportPopupOpen', false)
      this.$store.commit('report/setDeleteReportId', null)
      this.$store.commit('report/setDeleteReportType', null)
    ...
    deleteReportId () {
      this.$store.dispatch(
        'report/deleteReport',
        {
          'report_id': this.getDeleteReportId,
          'report_type': this.getDeleteReportType
        }
      ).then((response) => {
        this.closeDeleteReportModalPopUp()
        this.$emit('fetchReports', this.$event)
      })
      .catch(() => {})
    }
  },
